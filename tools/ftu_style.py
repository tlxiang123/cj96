...
COLORS = {
    ...
    "header": 0x0F6FCB,
    "primary": 0x168BFF,
    "primary_dark": 0x005BBB,
    "primary_soft": 0xD9EBFF,
    "cream": 0xCFE3FA,
    "cream_deep": 0xE9D5B8,
    "nav_dark": 0x126FC8,
    "amber": 0xFF9F0A,
...
FONT_PREVIEW_CAPTIONS = {
    "SysRunTextView", "SysRunShowTextView", "WaterBarTextView",
    "WaterBarShowTextView", "TrafficTextView", "TrafficShowTextView",
    "WaterSourceTextView", "WaterSource1TextView", "PumpTextView1",
    "PumpTextView2", "PumpTextView3", "PumpTextView4", "PumpTextView5",
    "WaterBarPicTextView1", "WaterBarPicTextView2",
}

PUHUITI_FAMILY = "Alibaba-PuHuiTi-Regular"
TEXT_CONTROL_KINDS = {
    "button", "textview", "edittext", "radiobuttons", "digitalclock",
    "item", "subitem", "checkbox",
}
...
def remove_controls(node: object, captions: set[str]) -> None:
    if isinstance(node, dict):
        for key, value in list(node.items()):
            if isinstance(value, dict) and value.get("caption") in captions:
                del node[key]
            else:
                remove_controls(value, captions)
...
def remove_font_preview() -> None:
    path = UI_DIR / "main.ftu"
    data, header, _ = decode_ftu(path)
    remove_controls(data, FONT_PREVIEW_CAPTIONS)
    path.write_bytes(encode_ftu(data, header))
    decoded, _, _ = decode_ftu(path)
    if any(caption in FONT_PREVIEW_CAPTIONS for caption, _ in control_signature(decoded)):
        raise RuntimeError("font preview controls were not fully removed")


def remove_gallery() -> None:
    path = UI_DIR / "main.ftu"
    data, header, _ = decode_ftu(path)
    remove_controls(data, {"Window7"})
    path.write_bytes(encode_ftu(data, header))
    decoded, _, _ = decode_ftu(path)
    remaining = {caption for caption, _ in control_signature(decoded)}
    if remaining.intersection({"Window7"}):
        raise RuntimeError("gallery controls were not fully removed")


def apply_font_family() -> None:
    for path in sorted(UI_DIR.glob("*.ftu")):
        data, header, _ = decode_ftu(path)

        def apply(node: object, kind: str = "root") -> None:
            if not isinstance(node, dict):
                return
            if kind in TEXT_CONTROL_KINDS:
                node["family"] = PUHUITI_FAMILY
            for key, value in node.items():
                child_kind = key.split("__", 1)[0] if "__" in key else key.lower()
                if isinstance(value, dict):
                    apply(value, child_kind)
                elif isinstance(value, list):
                    list_kind = "subitem" if key == "subItem" else child_kind
                    for child in value:
                        apply(child, list_kind)

        apply(data)
        path.write_bytes(encode_ftu(data, header))
        decoded, _, _ = decode_ftu(path)
        if decoded != data:
            raise RuntimeError(f"font round-trip verification failed for {path}")
        print(f"applied PuHuiTi to {path.name}")
...
def style_text(node: dict, in_header: bool) -> None:
    ...
    if in_header:
        node["colorTab"] = {"color0": COLORS["cream"]}
        return
...
def layout_main(data: dict) -> None:
    ...
    device_tip_pictures = {
        "AddressTipSubItem": "device_tip_address.png",
        "NameTipSubItem": "device_tip_name.png",
        "TypeTipSubItem": "device_tip_type.png",
        "ArreTipSubItem": "device_tip_group.png",
        "StatusTipSubItem": "device_tip_status.png",
    }

    def layout_device_tip_icons(node: object) -> None:
        if isinstance(node, dict):
            caption = node.get("caption")
            if caption == "DeviceTipListView":
                node["position"]["height"] = 99
                node.get("item", {}).get("position", {})["height"] = 94
            elif caption == "DeviceListView":
                node["position"]["top"] = 117
                node["position"]["height"] = 259
            elif caption in device_tip_pictures:
                node["text"] = ""
                node["picTab"] = {"pic0": device_tip_pictures[caption]}
                node["iconPosition"] = {
                    "height": 94,
                    "left": 28,
                    "top": 0,
                ...
                node["position"]["height"] = 94
                node["position"]["width"] = 151
            for value in node.values():
                layout_device_tip_icons(value)
        elif isinstance(node, list):
            for value in node:
                layout_device_tip_icons(value)

    layout_device_tip_icons(data)

    def layout_overview_children(node: dict) -> None:
        caption = node.get("caption")
        if caption and caption != "Window1":
            node["visible"] = False
            return
    ...
    for value in data.values():
        ...
        if caption == "Window1":
            ...
            remove_controls(value, {"WaterSourceImageAnim"})
        elif caption == "Window3":
            value["backgroundPic"] = "bgr_mid.png"

        if caption == "DigitalClock1":
            value["position"] = {
                "height": 48,
                "left": 846,
                "top": 18,
        ...
        if caption and caption.startswith("Button") and caption[6:].isdigit():
            index = int(caption[6:])
            if index in button_layouts:
                left, top, width, height = button_layouts[index]
...
def set_main_window_colors(data: dict) -> None:
    content_windows = {f"Window{index}" for index in range(1, 6)}
    data.pop("backgroundPic", None)
    data["backgroundColor"] = 0xFFFFFF

    def recolor(node: object, text_color: int, kind: str = "root") -> None:
        if not isinstance(node, dict):
            return
        if kind in TEXT_CONTROL_KINDS:
            if kind == "digitalclock":
                node["clockColor"] = text_color
            else:
                colors = node.get("colorTab", {"color0": text_color})
                node["colorTab"] = {key: text_color for key in colors}
        for key, value in node.items():
            child_kind = key.split("__", 1)[0] if "__" in key else key.lower()
            if isinstance(value, dict):
                recolor(value, text_color, child_kind)
            elif isinstance(value, list):
                list_kind = "subitem" if key == "subItem" else child_kind
                for child in value:
                    recolor(child, text_color, list_kind)

    def walk(node: object, kind: str = "root") -> None:
        if not isinstance(node, dict):
            return
        caption = node.get("caption")
        if caption in content_windows:
            node.pop("backgroundPic", None)
            node["backgroundColor"] = 0x000000
            recolor(node, 0xFFFFFF, kind)
            return
        if kind in TEXT_CONTROL_KINDS:
            recolor(node, 0x000000, kind)
            return
        for key, value in node.items():
            child_kind = key.split("__", 1)[0] if "__" in key else key.lower()
            if isinstance(value, dict):
                walk(value, child_kind)
            elif isinstance(value, list):
                list_kind = "subitem" if key == "subItem" else child_kind
                for child in value:
...
def preview_font(family: str, size: int) -> tuple[ImageFont.FreeTypeFont, bool]:
    for directory in FONT_DIRS:
        for name in FONT_PRESETS.get(family, ()):
            path = directory / name
            if path.exists():
                return ImageFont.truetype(str(path), size=size), True
    return font(size, family == "HarmonyOS Sans"), False
...
def make_tech_background() -> Image.Image:
    image = vertical_gradient((1024, 600), (34, 151, 235), (16, 105, 200))
    draw = ImageDraw.Draw(image)
    draw.rectangle((0, 0, 1023, 87), fill=rgb("header"))
    draw.rectangle((0, 500, 1023, 599), fill=(15, 101, 196))
    draw.rectangle((0, 87, 1023, 88), fill=(124, 191, 246))
    draw.rectangle((0, 500, 1023, 501), fill=(124, 191, 246))
    return image
...
def make_panel() -> Image.Image:
    ...
    panel = vertical_gradient((999, 425), rgb("cream"), rgb("cream_deep"))
    ...
    draw.rectangle((4, 3, 1002, 70), fill=(255, 250, 240, 190))
...
def make_overview_panel() -> Image.Image:
    ...
    draw.rounded_rectangle(
        (28, 28, 979, 372),
        12,
        fill=(255, 244, 226, 235),
        outline=rgb("border") + (255,),
        width=2,
    )
    for top in (16, 90, 164, 238, 312):
        draw.rounded_rectangle(
            (28, top, 979, top + 64),
            10,
            fill=(255, 250, 241, 245),
            outline=(22, 139, 255, 130),
            width=1,
        )
    return image.convert("RGB")
...
def make_nav_button(index: int, selected: bool) -> Image.Image:
    image = Image.new("RGBA", (94, 60), (0, 0, 0, 0))
    draw = ImageDraw.Draw(image)
    foreground = (0, 0, 0)
    draw.rounded_rectangle(
        (1, 1, 92, 58),
    ...
    if index <= len(icon_names):
        icon = render_lucide(icon_names[index - 1], 24, foreground)
        image.paste(icon, (35, 6), icon)
    else:
        icon_layer = Image.new("RGBA", (94, 60), (0, 0, 0, 0))
        draw_nav_icon(ImageDraw.Draw(icon_layer), index, foreground)
        image.paste(icon_layer, (0, 0), icon_layer)
    labels = ("总览", "设备", "计划", "测试", "调试", "", "记录", "告警")
    draw_centered_text(
        draw,
        (0, 36, 94, 56),
        labels[index - 1],
    ...
    if selected:
        draw.rounded_rectangle((36, 55, 58, 57), 1, fill=(0, 0, 0))
    return image
...
def main() -> None:
    ...
    parser.add_argument("--remove-font-preview", action="store_true")
    parser.add_argument("--apply-font", action="store_true")
    parser.add_argument("--remove-gallery", action="store_true")
    ...
    if args.remove_font_preview:
        remove_font_preview()
        print("removed font preview controls from main.ftu")
    ...
    if args.remove_gallery:
        remove_gallery()
        print("removed gallery controls from main.ftu")

    if not any((args.verify, args.apply, args.restore, args.inspect,
                args.remove_font_preview, args.apply_font, args.remove_gallery)):
