...
MatrixSearch::~MatrixSearch() {
  ...
  // add by guoxs
  if (SpellingTrie::hasInstance()) {
    delete &SpellingTrie::get_instance();
  }

  if (NGram::hasInstance()) {
    delete &NGram::get_instance();
  }

  if (LpiCache::hasInstance()) {
    delete &LpiCache::get_instance();
  }
  // add end
}
