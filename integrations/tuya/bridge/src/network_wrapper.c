...
int network_tls_connect(NetworkContext_t *pNetwork, const TLSConnectParams *params)
{
	...
	/* Keep the connection/handshake timeout separate from the online poll
	 * timeout.  The latter is changed after MQTT CONNECT succeeds. */
	tlsDataParams->read_timeout_ms = pNetwork->tlsConnectParams.timeout_ms;
	mbedtls_ssl_conf_read_timeout(&(tlsDataParams->conf), tlsDataParams->read_timeout_ms);
...
{
	if (pNetwork == NULL || pNetwork->context == NULL || timeout_ms == 0U) {
		return;
	}

	tls_context_t *tlsDataParams = (tls_context_t *)pNetwork->context;
	tlsDataParams->read_timeout_ms = timeout_ms;
	mbedtls_ssl_conf_read_timeout(&(tlsDataParams->conf), timeout_ms);
}
