...
void network_tls_set_read_timeout(NetworkContext_t *pNetwork, uint32_t timeout_ms);
...
mqtt_client_status_t mqtt_client_connect(void* client)
{
    ...
    /* 5000ms is for TCP/TLS/MQTT connection establishment only.  Once online,
     * the socket must use the short MQTT poll timeout so the bridge can keep
     * servicing RS485 and report queued frames promptly. */
    network_tls_set_read_timeout(&context->network, context->config.loop_timeout_ms);
...
mqtt_client_status_t mqtt_client_yield(void* client)
{
    ...
    uint32_t loop_timeout_ms = context->config.loop_timeout_ms;
    if (loop_timeout_ms == 0U) {
        loop_timeout_ms = context->config.timeout_ms;
    }

    mqtt_status = MQTT_ProcessLoop( &context->mqclient, loop_timeout_ms);
    if( mqtt_status != MQTTSuccess ) {
